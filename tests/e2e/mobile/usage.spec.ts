...
test.describe('שימוש אמיתי — בוקר', () => {
  test('קימה, שינה, הרגלים, בלוק עמוק, אימון, משימה, תכנון מחר, חדשות', async ({ page, errors }) => {
    ...
    await expect(page.locator('.focus-time')).toHaveText(/^(1:30:00|1:29:5\d)$/)
    ...
    await first.getByRole('button', { name: 'אהבתי', exact: true }).click()
    await expect(first.getByRole('button', { name: 'אהבתי', exact: true })).toHaveAttribute('aria-pressed', 'true')
    s = await readState(page)
    ...
    await news.locator('.item').nth(4).getByRole('button', { name: 'אהבתי', exact: true }).click()
