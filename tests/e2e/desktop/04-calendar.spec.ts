...
test.describe('יומן', () => {
  ...
  test('אירוע עם שעה: הוספה, מופיע ברשת, ברשימה ובמסך היום; עריכה; מחיקה', async ({ app }) => {
    ...
    await expect(ed.getByRole('group', { name: 'כותרת' }).locator('input')).toHaveValue('רופא שיניים')
    await ed.getByRole('group', { name: 'כותרת' }).locator('input').fill('רופא שיניים — נדחה')
