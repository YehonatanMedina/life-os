...
export async function assertNoHorizontalOverflow(page: Page, selector = '.chat-list') {
  ...
  expect(r.docScroll, `document scrollWidth ${r.docScroll} > clientWidth ${r.docClient}`).toBeLessThanOrEqual(r.docClient + 1)
...
export async function advance(page: Page, ms: number, step = 5_000) {
  let left = ms
  while (left > 0) {
