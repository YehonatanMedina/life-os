...
// steps מחליף את כל רשימת הצעדים. להוספת צעד — לשלוח את הרשימה הקיימת (מופיעה בהקשר) ועוד הצעד החדש, אחרת השאר נמחקים.
...
export function buildFastContext(s: AppState, now: number = Date.now()) {
  ...
  return {
    ...
    today: {
      ...
      wake: log.wake,
      // הצעדים נשלחים כדי ש-patchHabit על steps יוכל לשחזר את הרשימה המלאה ולא ימחק אותה
      habits: alive(s.habits).map((h) => ({ id: h.id, name: h.name, done: !!log.habits?.[h.id], steps: h.steps?.map((x) => x.text) })),
      workout: (() => {
