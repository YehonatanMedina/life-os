...
// -- ההקשר הדחוס -----------------------------------------------------------------
/** טקסט חופשי מקוצר להקשר — פסקה ארוכה לא אמורה לנפח כל תור */
function clip(t?: string, max = 1200): string | undefined {
  const v = t?.trim()
...
export function buildFastContext(s: AppState, now: number = Date.now()) {
  ...
  return {
    ...
    today: {
      ...
      wake: log.wake,
      // היומן של שגרת הערב — של הערב הזה ושל אתמול, מקוצר. זה מה שהוא עצמו כתב על היום.
      journal: clip(log.journal),
      journalYesterday: clip(dayLog(s, addDays(t, -1)).journal),
