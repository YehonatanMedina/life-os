...
export function planWeek(opts: {
  ...
  for (let d = 0; d <= 6; d++) {
    ...
    // **כשאין יום סטטיים נפרד, יום הדחיפה נושא את החשיפה השנייה.**
...
export function weekKinds(days: ProposedDay[]): DayKind[][] {
  return Array.from({ length: 7 }, (_, i) => {
    ...
    if (d.kind === 'gym' || d.kind === 'home') {
      // 'skills' הוא **עבודה סטטית בזרוע ישרה**, ולא "יום כוח". יום דחיפה
      // ויום רגליים לא נוגעים ב-Front Lever, ולכן הם לא נספרים בכלל
      // 48 השעות — אחרת ארבעה ימי כושר היו נראים כארבעה ימי סטטיים ברצף.
 ...
 *   1. **הסטטי בזרוע ישרה לפני הכוח בזרוע כפופה** — Front Lever נהרס
 *      מכתף וממרפק עייפים, והמיומנות עצמה כבר נעשתה בבית.
 *   2. **הכבד אחרי זה** — העלייה בכוח הגדולה ביותר היא בתרגילים
 *      שבתחילת האימון. pubmed.ncbi.nlm.nih.gov/32077380/
 *   3. **זוגות אנטגוניסטיים** (Front Lever מול פסאודו-פלאנש, מתח מול
 *      מקבילים) — אותה תוצאה בכ-30–50% פחות זמן, וההפסקה שרשומה היא
 *      המעבר לתרגיל השני ולא ההתאוששות המלאה.
 *      pubmed.ncbi.nlm.nih.gov/20733520/
 *
 * והתקרות שקובעות כמה נכנס: `WEEKLY_SETS` (משיכה 12–16, דחיפה 10–14) —
 * כשאחיזת Front Lever נספרת כסט משיכה לכל דבר. תשעה סטים של Front Lever
 * בשבוע כבר לוקחים יותר מחצי מתקציב המשיכה, וזו הסיבה שאין כאן משיכת
 * פולי או חתירה נוספת.
 */
export const ROLE_BLOCK: Record<Role, { title: string; focus: string; how: string; exercises: ProposedExercise[] }> = {
  pull: {
    title: 'משיכה וסטטיים',
    focus: 'היום של Front Lever — סטטי כשהזרוע טרייה, ואז המשיכה הכבדה',
    how: 'הסטטי לפני המתח ולא אחריו: תרגול תחת עייפות מקודד זיכרון מוטורי גרוע.',
    exercises: [
      handstand(3),
      wrists(),
      {
      ...
      },
      {
        ...
        why: 'המדד המרכזי: מתח בתוספת 40–50% ממשקל הגוף הוא השער גם ל-Front Lever וגם למאסל־אפ.',
      },
  ...
  statics: {
    title: 'סטטיים וליבה',
    focus: 'החשיפה השנייה ל-Front Lever בשבוע, סאב-מקסימלית, ועבודת הליבה',
    how: 'הכי קל בשבוע במכוון: הגיד מסתגל לאט מהשריר, ושתי חשיפות מקסימליות בשבוע הן עומס בלי הסתגלות.',
    exercises: [
      handstand(3),
      wrists(),
      {
        ...
        metric: 'time',
        rest: 60,
        note: 'סאב-מקסימלי, כ-60% מהאחיזה המקסימלית. 72 שעות מהיום הראשון.',
        why: 'פעמיים בשבוע לכל דפוס עדיף על פעם אחת בנפח זהה; שני ימים ברצף הם עומס בלי הסתגלות.',
      },
      { name: 'חתירה במשקולות בודדות או במכונה', sets: 3, reps: '8-10 לכל יד', metric: 'weight', rest: 75, note: 'משיכה אופקית בזרוע כפופה — הווקטור השני.' },
  ...
  push: {
    ...
    how: 'בלי סטטיים היום: זה היום שמפריד בין שתי חשיפות ה-Front Lever.',
    exercises: [
      handstand(3),
      hollow(),
...
/**
 * החשיפה השנייה ל-Front Lever, כשאין לה יום משלה. ראו `planWeek`.
 */
export const SECOND_LEVER: ProposedExercise = {
  name: 'Front Lever — Tuck',
  sets: 4,
  reps: '8 שניות',
  metric: 'time',
  rest: 60,
  note: 'סאב-מקסימלי, כ-60% מהאחיזה המקסימלית. זו החשיפה השנייה בשבוע, ובלעדיה הכל תחזוקה.',
...
export const HOME_TWIN: Record<Role, { title: string; exercises: ProposedExercise[] }> = {
  pull: {
    ...
    exercises: [
      ...
      { name: 'שכיבות סמיכה פסאודו-פלאנש', sets: 3, reps: '6-8', metric: 'bodyweight', rest: 60 },
