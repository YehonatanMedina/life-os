 ...
 *    מוט. הם ישבו לפניו עד 22.9.2026, וזה היה סדר של טעם ולא של תנאי.
...
export const GOAL_ORDER: string[] = [
  ...
  'sk-pullup',
  'sk-dip',
  // Front Lever עלה לכאן ב-22.9.2026, ולא בגלל שהוא נעשה קל יותר:
  // **התוכנית מאמנת אותו פעמיים בשבוע, תשעה סטים.** רשימת מוקד שלא
  // כוללת את המיומנות הכי מאומנת בשבוע היא לא רשימת מוקד. הכלל שלפיו
  // מה שקובע מקום הוא מתי אפשר לאמן את השלב הראשון — ו-`needs` שלו אומר
  // "כלום מלבד מוט" — הוא שמכריע כאן, ולא הסדר שהיה.
  'sk-frontlever',
 ...
 * המטרות, והמוקד רק אומר לאן הולכת תשומת הלב השבוע. שש, כי אלה בדיוק
 * המטרות שיש להן תרגיל בתוכנית השבועית שמודד אותן כל שבוע — הריצה,
 * עמידת ידיים, L-Sit, מתח, מקבילים ו-Front Lever. מטרה שיוצאת מהתוכנית
 * יורדת בסדר, ומטרה שנכנסת אליו עולה: המוקד אמור לתאר את מה שקורה
 * בפועל, ולא רשימה שנכתבה פעם אחת.
 */
