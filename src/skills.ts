 ...
 *    מתח לפני מתח ביד אחת, עמידת ידיים לפני HSPU, L-Sit לפני V-Sit.
 *    **Back Lever ודגל הדרקון אינם תנאי ל-Front Lever** — הם משלימים לו,
 *    ו-`needs` של Front Lever אומר במפורש שלשלב הראשון לא צריך כלום מלבד
 *    מוט. ולכן המקום שלו ביחס אליהם הוא החלטה, לא חוק: הוא עבר לפניהם
 *    ב-22.9.2026 כשהתוכנית אימנה אותו פעמיים בשבוע, וחזר אחריהם ב-23.9
 *    כשהיא הפסיקה.
...
export const GOAL_ORDER: string[] = [
  ...
  'sk-dragonflag',
  'sk-muscleup',
  // Front Lever ירד לכאן ב-23.9.2026, ולא בגלל שהוא נעשה קשה יותר:
  // **התוכנית הפסיקה לאמן אותו.** הוא עלה למוקד ב-22.9 כי היו לו תשעה
  // סטים בשבוע, וכשהסטים ירדו מהתוכנית הכלל עובד לשני הכיוונים — מטרה
  // שיוצאת מהתוכנית יורדת בסדר. הסדר כאן הוא סדר של התקדמות: החזקות
  // בזרוע ישרה שקלות ממנו (Back Lever, דגל הדרקון) והמאסל־אפ שיושב
  // ישירות על המתח והמקבילים — לפניו.
  'sk-frontlever',
  'sk-pistol',
  'sk-oapushup',
  'sk-hspu',
 ...
 * ומטרה שנכנסת אליו עולה: המוקד אמור לתאר את מה שקורה בפועל, ולא
 * רשימה שנכתבה פעם אחת. (חזר מ-6 ל-5 ב-23.9.2026, כש-Front Lever ירד
 * מהתוכנית.)
 */
...
/**
 * קצב הגידול השבועי שהתוכנית מתכננת לפיו.
 *
 * **זה לא כלל בטיחות, וחשוב לא להציג אותו ככזה.** הניסוי המבוקר היחיד
 * שבדק את "כלל ה-10%" מצא 20.8% פציעות מול 20.3%, p=0.90
 * (pubmed.ncbi.nlm.nih.gov/17940147/). מה שכן נמדד, על 874 רצים, הוא
 * **עלייה של מעל 30% על פני שבועיים** (jospt.org/doi/10.2519/jospt.2014.5164),
 * ועל 5,205 רצים — **אימון בודד שחורג מהארוך ביותר ב-30 הימים האחרונים**
 * (pmc.ncbi.nlm.nih.gov/articles/PMC12421110/). שני אלה הם מה שהאפליקציה
 * מזהירה עליהם; 10% הם רק הקצב שתוכניות אמיתיות בונות לפיו (9%±4).
 */
